...
import policeStations from '../organizations/police.json';
...
function PoliceDashboard() {
  ...
  const policeStation = policeStations.find(station => station.id === parseInt(id));

  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/organization/police/${id}/dashboard`, {
          params: {
            lat: policeStation.latitude,
            lng: policeStation.longitude
          }
        });
  ...
  if (!policeStation) {
    return <div className="p-8">Police station not found</div>;
  }
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">{policeStation.name} Dashboard</h1>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <p className="text-gray-600">Lat: {policeStation.latitude}</p>
            <p className="text-gray-600">Long: {policeStation.longitude}</p>
          </div>
        </div>
